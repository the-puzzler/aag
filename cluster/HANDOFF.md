...
- **09-30 ~14:00 — aag219 (CelebA critic + SQUARED-HINGE floored cycle, fixed 0.1, 160 ep):** FID-10k 29.3 → **15.42 @552 / 15.50 @560**, monotone, no collapse (df 0.55 at the end vs the global critic's late decoupling). Critic alone (aag160): 15.71 @560 → the first cycle variant at parity-or-better at 10k. Fresh cycle error 0.585 → 0.51 vs anchor 0.43: excess 0.15 → 0.08, reduced gently (gradient ∝ excess), not eliminated. FID-50k queued (aag160 = 14.62). aag220 (MMD + squared-hinge cycle) running.
- **09-30 ~15:00:** aag219 FID-50k **14.54** (critic + squared-hinge cycle, fixed 0.1) vs critic alone **14.62** — parity (within noise) with a monotone, non-collapsing curve. CelebA critic row: 14.62 | +cycle 32.13 | +cycle-floored-adaptive 15.91 | +cycle-squared-hinge-fixed **14.54**. The pointwise term's value at this E quality (R² 0.50) is stability, not a lower optimum. aag220 (MMD + squared-hinge) running.
- **09-30 ~17:00 — aag220 (CelebA MMD + SQUARED-HINGE floored cycle, fixed 0.1, 40 ep): FID-10k 31.4 → 18.89 @440, still falling** vs global MMD alone 20.24 @440 (−1.35), unfloored 23.23, adaptive-floored 20.76. MMD statistic 0.0098 ≈ control 0.0095 (no longer slowed); fresh cycle error 0.59 → 0.52 (anchor 0.43). First clear GAIN from the pointwise term. FID-50k queued (control 19.38). aag221 (ImageNet MMD mean-stop + squared-hinge cycle) running.
- **09-30 ~18:00:** aag220 FID-50k **18.13** (MMD + squared-hinge cycle) vs global MMD **19.38** (−1.25). CelebA MMD row: 19.38 | +cycle 22.48 | +cycle-floored-adaptive 20.11 | +cycle-squared-hinge-fixed **18.13**. First win for the inverse-cycle objective. aag221 (ImageNet, E R² 0.27) started: fresh 0.745 vs anchor 0.728 → excess 0.017, essentially inert.
- **09-30 ~19:00 — apply the winning cycle recipe to the CelebA BEST:** aag222 = wide hybrid 135M (aag173 plain ep800) + big critic 160 ep + squared-hinge floored cycle (fixed 0.1), E = DINO head on the hybrid's 64-d code (trained locally). Control aag174 = **14.19** (current CelebA best). Inserted after aag221; then 211 rerun, 204, 205 (chain90).
- **10-01 ~01:30 — aag221 (ImageNet w1.9 MMD mean-stop + squared-hinge cycle fixed 0.1, E R² 0.27):** FID-10k 23.30 @17 → 26.25 → **27.11 @19** (stop at step 90,228, one epoch later than aag206's 84,735; ts_clamp 9% at ep17 vs 47%). aag206 without cycle: 23.20 → 22.93. Fresh cycle error 0.745–0.76 vs anchor 0.727: excess ≈ 0.02 (nothing to remove), yet the term still hurt: on ImageNet ‖∇cyc‖ ≈ 140×‖∇sup‖ (512-d E output through DINO; the adaptive multipliers of aag217 gave the ratio) vs ≈ 9× on CelebA, so fixed 0.1 × 2·0.02 · ∇cyc ≈ 0.5 ‖∇sup‖ of noise. Verdict: with E at R² 0.27 the cycle test on ImageNet is inert-to-harmful; a much stronger E (TiTok-latent inverse) is a prerequisite, and the weight should be set from the gradient-norm ratio once at the start (per dataset) rather than a fixed constant. FID-50k queued. aag222 (CelebA best + cycle) next.
- **10-01 ~03:00:** aag221 FID-50k **24.44** (ImageNet MMD mean-stop + squared-hinge cycle, weak E) vs aag206 20.24. ImageNet cycle row closed: needs a strong E first.
- **10-01 ~06:00 — aag222 (wide hybrid + critic + squared-hinge cycle, 160 ep from aag173 ep800):** FID-10k 33.4 → **15.48 @960**, still falling, critic balanced (df 0.60). Control aag174 (no cycle): 15.17 @960 (FID-50k 14.19). ≈0.3 behind at 10k — parity-ish, as with the conv critic (15.50 vs 15.71). Fresh cycle error 0.56 → 0.49 vs anchor 0.41 (excess 0.15 → 0.08). FID-50k queued. aag211 (cycle alone) rerun starting; then w2.5.
- **10-01 ~07:00:** aag222 FID-50k **14.55** (wide hybrid + critic + squared-hinge cycle) vs aag174 **14.19** (no cycle). The cycle term does not improve the CelebA best; parity-to-slightly-worse under the critic on both architectures (conv 14.54 vs 14.62; hybrid 14.55 vs 14.19). Its only clear win remains MMD on CelebA (18.13 vs 19.38), i.e. it helps where the distributional term is weaker.
- **10-01 ~08:00 — aag211 (CelebA cycle ALONE, unfloored, 40 ep):** FID-10k 35.1 → min 31.28 @420 → 36.15 @440, fresh cycle error 0.58 → 0.30 (anchor 0.39–0.44): the raw term alone lowers FID only a little (base ≈ 30–34) and then exploits E; confirms the raw cycle term is not a standalone off-anchor objective. FID-50k queued. aag204 (w2.5 plain 16 ep, 257M) running (~18 h), then aag205.
- **10-01 ~09:00:** aag211 FID-50k **35.22** (cycle alone, unfloored). Full CelebA cycle table (FID-50k, base aag159 r64 w1.4): none (plain) ≈ 30 | cycle alone 35.22 | critic 14.62 → +cycle 32.13 / +floored-adaptive 15.91 / +squared-hinge **14.54** | MMD 19.38 → +cycle 22.48 / +floored-adaptive 20.11 / +squared-hinge **18.13**; hybrid best 14.19 → +squared-hinge 14.55. ImageNet w1.9: MMD mean-stop 20.24 → +floored-adaptive 41.51 / +squared-hinge 24.44 (E R² 0.27). Pending: aag204/205 (w2.5).
- **10-01 ~10:00 — aag204 (w2.5 plain) PREEMPTED at 06:55 UTC** by the cluster controller ("freeing 1 nodes for entitled job agora-acheron-exp-055h2; its project over guarantee") — our jobs run at priority class `build`. Pod deleted, job set to `suspend: true`. Not counting on auto-resume (another job has been suspended since 09-03) → `unsuspend_watch.sh` re-enables aag204/aag205 when found suspended (10 min grace); the trainer resumes from the last checkpoint. Expect the w2.5 timeline to slip by however long the entitled job holds the node.
- **10-02 ~00:00:** aag204 (ImageNet flat conv **w2.5, 257M**, plain 16 ep on the AAG3 K=1 floor; preempted 3× by the cluster, resumed from checkpoints each time) done: ep6 100.1 → ep12 113.9 → **116.2 @16** (MSE 0.0556 @12); same drift shape as w1.9 (114.0 @16). aag205 = w2.5 + global MMD, MEAN floor-stop, from ep16 — running (control: w1.9 aag206 20.24).
- **10-02 ~02:30 — aag205 (w2.5 257M + global MMD, mean floor-stop):** stopped at step 84,071 (0.97 epoch into the MMD stage; ts_clamp 51%), fresh FID-10k **23.73** at the stop. w1.9 (aag206) stopped at step 84,735 with 22.93. Width 1.9 → 2.5 does not help at the MMD stop (10k). FID-50k queued (w1.9: 20.24).
- **10-02 ~04:00 — aag205 FID-50k 21.05** (w2.5 257M + MMD mean-stop, last ckpt) vs w1.9 150M **20.24** (aag206), w1.4 85M 25.0. Capacity gain saturates between w1.9 and w2.5 under MMD on ImageNet (w1.9 → w2.5: −0.8, within run-to-run noise of ~±0.3 but the wrong sign). ImageNet best stays **20.24 @150M**; the 310M budget is not the lever now — the off-anchor objective is. Queue empty; node idle. "Recommended steps" round complete: (1) mean floor-stop → 20.24 banked as LAST ckpt; (2) pairwise critic after MMD harmful (44.5); (3) w2.5 no gain (21.05).
- **10-02 — user: train the inverse FROM SCRATCH as a true inverse of the generator** ("they aren't TiTok latents any more once the assignment has shuffled them, that's why I'd rather train the inverse from scratch"). Right call: `aag/inverse_conv.py` = mirror of Generator256 (five stride-2 stages 256→8, residual blocks, pool or grid head), unconditional, trained on the real assigned pairs only by the same `scripts/train_inverse256.py` (`--encoder conv|dino`, `--enc-width/--enc-res/--enc-head/--enc-dropout/--ema`). First CelebA run: **held-out R² 0.50 at epoch 3, 0.59 by epoch 5** with 27.1M params and 30 s/epoch — the DINO-head ceiling (0.497, any head or schedule) was a property of the FEATURES, not of the task. Sweep running (w1/20ep, w1/60ep+EMA, w0.5/40ep, w1/40ep+dropout+EMA).
  Also added `--cycle-weight-auto r --cycle-auto-steps N`: calibrate the cycle weight ONCE from the median gradient-norm ratio of the hinged cycle term to the supervised loss, then freeze, so the push starts at r× the supervised gradient and decays with the excess. This replaces the fixed 0.1 that could not transfer (the ratio is ~9× on CelebA and ~140× on ImageNet). r = 0.25 reproduces the CelebA winner's starting push.
  Runs (chain91): **aag223** ImageNet conv E (full z, 8 ep, EMA), **aag224** CelebA MMD + conv-cycle (controls 19.38 / DINO-cycle 18.13), **aag225** CelebA critic + conv-cycle (14.62 / 14.54), **aag226** ImageNet MMD mean-stop + conv-cycle (20.24 / DINO-cycle 24.44).
- **10-02 — FROM-SCRATCH CONV INVERSE (aag/inverse_conv.py), CelebA E: held-out R² 0.770** (DINO head 0.497 regardless of size/schedule → that ceiling was the FEATURES, not the task). Sweep: w1/20ep 0.754, w1/60ep+EMA 0.774 best, w0.5/40ep 0.749 (6.8M params!), w1/40ep+dropout+EMA 0.771; every config ≈0.75 → task property, not capacity. Fixed two bugs: the script saved the LAST epoch while I ranked by the BEST (now saves best), and `--cycle-weight-auto r` calibrates the weight once from the median gradient-norm ratio (cluster: 0.182 × 0.25 → w 0.045; local single-process: 0.430 × 0.25 → 0.108 — the ratio is run-dependent, which is why the old fixed weight could not transfer).
- **10-02 — aag224 (CelebA MMD + conv-E cycle, 40 ep):** FID-10k 33.79 → **18.90 @440**, still falling; DINO-E cycle (aag220) 18.89 @440, global MMD control (aag191) 20.24. Cycle error: fresh 0.382 → 0.310 vs anchor floor 0.187–0.198 (the stronger E halves the anchor error: 0.19 vs 0.43 with DINO-E, and the fresh/anchor RATIO rises 1.36× → 1.93×, i.e. much more signal). MMD statistic 0.0098 = the control's, so the cycle no longer slows the distributional term. **Identical FID to the weak-E cycle despite 1.5× the R² and 2× the relative signal** → the gain is not limited by inverse quality. FID-50k queued (DINO-E 18.13). aag225 (critic + conv-E cycle) next.
- **10-02:** aag224 FID-50k **17.96** (MMD + conv-E cycle) vs DINO-E cycle **18.13**, global MMD **19.38**. CelebA MMD row: none 19.38 | weak-E cycle 18.13 | strong-E cycle 17.96. Tripling the inverse's explained variance (0.50 → 0.77) buys 0.17 FID on top of the 1.25 the cycle term already gave — the pointwise term's value is essentially independent of E quality above R²≈0.5.
- **10-02 — preemption-safe resume.** The cluster preempted our jobs ~5× today (other projects' entitled jobs). Finetune configs pin `--resume $RESUME` (a base checkpoint), so a preempted finetune RESTARTED FROM THE BASE: aag225 silently threw away 112 epochs. Fix: `--resume auto:<base>` = latest `gen_ep*.pt` under `--out` if present, else `<base>` (commit 4b6cc14); applied to the cycle configs. aag225 resubmitted and confirmed resuming from its own `gen_ep512.pt`. **Every preemptible finetune should use `auto:` from now on.**
- **10-02 — aag225 (CelebA critic + conv-E cycle, 160 ep):** FID-10k **15.76 @560** (min 15.71 @552), monotone, critic balanced to the end (df 0.54, no late decoupling). Controls: critic alone 15.71 @560 (FID-50k 14.62), DINO-E cycle 15.50 (14.54). Fresh cycle error 0.302 → 0.277 vs anchor 0.193–0.201. So on the CRITIC route the strong inverse is also a wash: same FID as no cycle at all, same as the weak-E cycle. FID-50k queued.
- **10-02 — CONV-INVERSE CYCLE ROUND COMPLETE (CelebA, FID-50k, base aag159 r64 w1.4 K=1-floor plain ep400):**
  | off-anchor objective | no cycle | + cycle, DINO E (R² 0.50) | + cycle, conv E (R² 0.77) |
  |---|---|---|---|
  | global MMD, 40 ep | 19.38 | 18.13 | **17.96** |
  | global critic, 160 ep | **14.62** | 14.54 | 14.57 |
  Encoder quality is NOT the lever: R² 0.50 → 0.77 (anchor floor 0.43 → 0.19, fresh/anchor ratio 1.36× → 1.93×) moves FID by 0.17 on MMD and 0.03 on the critic. The cycle term is worth ~1.4 FID where the distributional term is weak (MMD) and ~0 where it is strong (critic), and that split is the same at both encoder qualities. Its reliable benefit on the critic route is stability (monotone curve, critic balanced at ep560, no late decoupling) rather than a lower optimum. ImageNet conv-E (aag223) and its cycle run (aag226) remain HELD at the user's request.
