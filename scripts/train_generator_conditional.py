...
from aag.ae import ResidualDecoder as ConvDecoder, ResidualDecoder3d
from aag.datasets import spec
from aag.video import lpips_any, load_video_targets, save_video_grid
...
def main():
    ...
    from aag.datasets import collect_conditions
    if args.dataset == "celeba":
        attrs = torch.load("results_celeba/attrs.pt", map_location=device, weights_only=False)
        cond = attrs["attrs"].to(device).float()
        attr_names = attrs["attr_names"]
    else:
        cond, _ = collect_conditions(args.dataset, args.data, args.batch, z.shape[0], device=device)
        attr_names = [f"class_{i}" for i in range(cond.shape[1])]
    N, dim_z = z.shape
    ...
    if VIDEO:
        targets = load_video_targets(args.data, N, device=device)   # uint8 on GPU
    else:
        from aag.datasets import get_loaders
        _, enc_loader, _, n_avail = get_loaders(
            args.dataset, args.data, args.batch, n_particles=N, image_size=args.image_size,
        )
        targets = torch.cat([x for x, _ in enc_loader], 0).to(device)
    ...
    if VIDEO:
        model = ResidualDecoder3d(dim_z + n_attrs, ch=args.ch, image_size=args.image_size,
                                  frames=spec(args.dataset)["frames"]).to(device)
    else:
    ...
    if args.resume is not None:
        ...
        log(f"resumed from {args.resume} (was epoch {rk.get('epoch')})")
    ...
    def run(idx, train):
        ...
        for i in range(0, order.numel(), args.batch):
            ...
            with torch.set_grad_enabled(train):
                ...
                perc = lpips_any(perceptual, pred.clamp(-1, 1), targets[b]).mean()
    ...
    for ep in range(args.start_epoch, args.epochs):
        ...
        if (ep + 1) % args.eval_every == 0 or ep == args.epochs - 1:
            ...
            _p = ckpt_dir / f"generator_ep{ep+1}.pt"
            torch.save({
    ...
    n_cols = 8
    conds_demo, labels = [], []
    combos = ([[], ["Male"], ["Smiling"], ["Blond_Hair"], ["Male", "Eyeglasses"]]
               if args.dataset == "celeba" else [[n] for n in attr_names[:5]])
    for want_on in combos:
    ...
    (save_video_grid(grid, args.out / "demo_fixed_condition_varying_z.png") if VIDEO else
    ...
    toggle_attrs = (["Male", "Smiling", "Blond_Hair", "Eyeglasses", "Wearing_Hat", "Young", "No_Beard"]
                    if args.dataset == "celeba" else attr_names[:7])
    torch.manual_seed(1)
    n_z_rows = 4
    with torch.no_grad():
        rows = []
