...
def collect_targets(data_dir, n, image_size, particle_order=False, dataset="celeba"):
    ...
    particle_order=True: aag.celeba_data.celeba_loaders' permuted particle subset
    ...
    if particle_order:
        ...
        _, enc_loader, _, _ = get_loaders(dataset, data_dir, 256, n_particles=n, image_size=image_size)
        imgs = []
...
def main():
    ...
    ap.add_argument("--particle-order", action="store_true",
                    help="use aag.celeba_data's permuted particle order for targets "
    ...
    if VIDEO:
        ...
        targets = collect_targets(args.data, N, args.image_size,
    ...
    if VIDEO:
        model = ResidualDecoder3d(dim, ch=args.ch, image_size=args.image_size,
    ...
    for tag, ep in [("lpips", best_epoch_lpips), ("mse", best_epoch_mse)]:
        ...
        with torch.no_grad():
            ...
            imgs = model(zs)
        if VIDEO:
            save_video_grid(imgs, args.out / f"samples_best_{tag}_ep{ep}.png")
        else:
